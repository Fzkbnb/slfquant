...
INSERT INTO `t_menu` VALUES (70, 'menu-7', '策略管理', 1, 1, NULL);
INSERT INTO `t_menu` VALUES (7010, 'menu-71', 'api配置', 70, 0, '/strategy/quantApiConfig.html');
INSERT INTO `t_menu` VALUES (7020, 'menu-72', '网格配置', 70, 0, '/strategy/quantGridConfig.html');
INSERT INTO `t_menu` VALUES (7030, 'menu-73', '网格交易统计', 70, 0, '/strategy/quantGridStats.html');


INSERT INTO `t_role_menu` VALUES (128, 7010, 1);
INSERT INTO `t_role_menu` VALUES (129, 7020, 1);
INSERT INTO `t_role_menu` VALUES (130, 7030, 1);
INSERT INTO `t_role_menu` VALUES (131, 70, 1);
...
(
  ...
  buy_usdt      decimal(16, 8),
  sell_usdt     decimal(16, 8),
  ...
  first_direct  VARCHAR(16) NOT NULL,
  primary key (id)
);
DROP TABLE IF EXISTS `quant_grid_config`;
CREATE TABLE `quant_grid_config`
(
  ...
  `first_direct`     varchar(255)   NOT NULL COMMENT '首笔交易方向',
  `entrust_cont`     int            NOT NULL COMMENT '单笔委托张数',
  `enable_maker`     int DEFAULT 1 COMMENT 'maker开关',
  `price_diff_rate`  decimal(16, 8) NOT NULL COMMENT '网格价差率',
  `profit_rate`      decimal(16, 8) NOT NULL COMMENT '止盈价差率',
  `max_buy_price`    decimal(16, 8) NOT NULL COMMENT '最大买入价格',
  `min_sell_price`   decimal(16, 8) NOT NULL COMMENT '最小卖出价格',
  `fee_rate`         decimal(16, 8) NOT NULL COMMENT '手续费率',
  `lever_rate`       decimal(16, 8) NOT NULL COMMENT '杠杆率',
...
(
  ...
  `pass_phrase`      varchar(255) NOT NULL COMMENT 'pass_phrase',
  `auth_type`        int          NOT NULL COMMENT '权限类型（1：只读；2：交易）',
  `divisor`          varchar(255) NOT NULL COMMENT '用户加密因子',
  `remark`           varchar(255) DEFAULT NULL COMMENT '备注',
  `create_time`      bigint       NOT NULL COMMENT '创建日期',
  `update_time`      bigint       NOT NULL COMMENT '更新时间',
  PRIMARY KEY (`id`)
) ROW_FORMAT = COMPACT COMMENT ='api配置表';

DROP TABLE IF EXISTS `quant_avg_config`;
CREATE TABLE `quant_avg_config`
(
  `id`                 bigint COMMENT 'ID',
  `account_id`         bigint         NOT NULL COMMENT '用户ID',
  `exchange`           varchar(255)   NOT NULL COMMENT '交易所',
  ...
  `usdt_balance`       decimal(24, 8) NOT NULL COMMENT 'usdt余额',
  `coin_balance`       decimal(24, 8) NOT NULL COMMENT 'coin余额',
