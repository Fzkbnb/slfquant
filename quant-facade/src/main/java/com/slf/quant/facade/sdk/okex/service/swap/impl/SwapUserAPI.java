...
public interface SwapUserAPI {
    ...
    Call<String> selectOrders(@Path("instrument_id") String instrumentId, @Query("state") String state, @Query("before") String before, @Query("after") String after, @Query("limit") String limit);
    //获取订单信息
    @GET("/api/swap/v3/orders/{instrument_id}/{order_id}")
    Call<String> selectOrderByOrderId(@Path("instrument_id") String instrumentId, @Path("order_id") String orderId);
    @GET("/api/swap/v3/orders/{instrument_id}/{client_oid}")
    Call<String> selectOrderByClientOid(@Path("instrument_id") String instrumentId, @Path("client_oid") String client_oid);
    ...
    Call<String> getLedger(@Path("instrument_id") String instrumentId, @Query("before") String before, @Query("after") String after, @Query("limit") String limit, @Query("type") String type);
