...
import com.slf.quant.facade.sdk.okex.bean.swap.param.CancelOrderAlgo;
import com.slf.quant.facade.sdk.okex.bean.swap.param.PpOrder;
import com.slf.quant.facade.sdk.okex.bean.swap.param.SwapOrderParam;
...
public interface SwapTradeAPI
{
    ...
    Call<String> getSwapOrders(@Path("instrument_id") String instrument_id, @Query("order_type") String order_type, @Query("status") String status,
            @Query("algo_id") String algo_id, @Query("before") String before, @Query("after") String after, @Query("limit") String limit);
    
    // 市价全平
    @POST("/api/swap/v3/close_position")
    Call<JSONObject> closePosition(@Body JSONObject param);
}
